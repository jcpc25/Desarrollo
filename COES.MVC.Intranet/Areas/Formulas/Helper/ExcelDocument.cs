...
namespace COES.MVC.Intranet.Areas.Formulas.Helper
{
    public class ExcelDocument
    {
        ...
        public static void GenerarExcelPerfilScada(List<PerfilScadaDTO> list)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(newFile, template))
            {
                ...
                if (ws != null)
                {
                    ...
                    {                       
                        ws.Cells[3, column].Value = item.LASTUSER.ToUpper();
                        ws.Cells[3, column].StyleID = ws.Cells[3, 1].StyleID;
                        ws.Cells[4, column].Value = item.PRRUABREV;
                        ws.Cells[4, column].StyleID = ws.Cells[3, 1].StyleID;
                        ws.Cells[5, column].Value = item.PRRUNOMB;                        
        ...
        /// <param name="?"></param>
        /// <param name="?"></param>
        public static void GenerarPlantillaImportacion(string fuente, string subEstacion, string area, string agrupacion, int formula)
        {
            string ruta = ConfigurationManager.AppSettings[RutaDirectorio.ReportePerfiles].ToString();
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(newFile, template))
            {
                ...
                ws.Cells[2, 2].Value = fuente;
                ws.Cells[3, 2].Value = subEstacion;
                ws.Cells[4, 2].Value = area;
                ws.Cells[5, 2].Value = agrupacion;
        ...
        public static ScadaDTO ImportarDatos(string file, out int codigo)
        {
            try
            {
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
                {
                    ...
                    {
                        if (int.TryParse(ws.Cells[1, 2].ToString(), out codigo)) { }
                    }
