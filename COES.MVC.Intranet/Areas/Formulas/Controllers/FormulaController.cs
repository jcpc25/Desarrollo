...
namespace COES.MVC.Intranet.Areas.Formulas.Controllers
{
    public class FormulaController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public PartialViewResult Formula(string fuente)
        {
            FormulaModel model = new FormulaModel();
            int areaCode = (int)((COES.MVC.Intranet.SeguridadServicio.UserDTO)Session[DatosSesion.SesionUsuario]).AreaCode;

            List<MePerfilRuleDTO> formulas = this.servicio.GetByCriteriaMePerfilRules(areaCode, fuente);      
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Grid(string fechaInicio, string fechaFin, string fuente)
        {
            FormulaModel model = new FormulaModel();
            ...
            model.Listado = this.servicio.ListarPerfilesPorUsuario(User.Identity.Name, inicio, fin, fuente);          
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GenerarArchivo(string fuente)
        {
            int result = 1;
            try
            {
                int areaCode = (int)((COES.MVC.Intranet.SeguridadServicio.UserDTO)Session[DatosSesion.SesionUsuario]).AreaCode;
                List<PerfilScadaDTO> list = this.servicio.ObtenerPerfilesExportacion(User.Identity.Name, areaCode, fuente);
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GenerarFormato(string fuente, string subEstacion, string area, string agrupacion, int formula)
        {
            int result = 1;
            try
            {
                ExcelDocument.GenerarPlantillaImportacion(fuente, subEstacion, area, agrupacion, formula);
