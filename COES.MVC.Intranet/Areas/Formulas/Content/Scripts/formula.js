...
function cargarFormulas()
{
    ...
    if (fuente != "") {
        ...
        mostrarAlerta("Por favor seleccione fuente");
    }
}
...
{
    var fuente = $('#hfFuente').val();
    if (fuente == "E") fuente = "Ejecutado";
    if (fuente == "D") fuente = "Demanda en barras";

    $.ajax({
        type: 'POST',
        url: controlador + 'formula/generarformato',
        data: {
            fuente: fuente, subEstacion: $('#hfSubEstacion').val(),
            area: $('#hfArea').val(), agrupacion: $('#cbAgrupacion :selected').text(), formula: $('#hfIdFormula').val()
        },
        dataType: 'json',
