...
$(function () {
    ...
    $('#cbFamilia').multipleSelect({
        ...
        onClose: function (view) {
            ...
            var totalTipoEquipos = tipoEquipo.toString().split(",");
            if (totalTipoEquipos.length > 5 || totalTipoEquipos == "") {
                $('#equipos').css('display', 'none');
                consideraEquipo = false;
            }
            else {
                $('#equipos').css('display', 'block');
                consideraEquipo = true;
                cargarEquipos();
            }
        }
    });
...
function buscarEvento() {
    ...
    if (equipo != "[object Object]") ;
    {
        //alert(equipo);
        var totalEquipos = equipo.toString().split(",");
        if (totalEquipos.length > 50) {
            alert("Seleccionar menos de 50 equipos");
            return;
        }
    }
    pintarPaginado();
...
function cargarEquipos() {
    ...
    $('#hfTipoEquipo').val(familia);
    var empresa = $('#cbEmpresa').multipleSelect('getSelects');
    if (empresa == "[object Object]") empresa = "-1";
    $('#hfEmpresa').val(empresa);
    $.ajax({
        type: 'POST',
        url: controlador + 'mantenimiento/Equipos',

        data: { familia: $('#hfTipoEquipo').val() ,empresas:$('#hfEmpresa').val()},
...
function exportar_ManttoEmpresa()
{  // Genera Reporte excel de Mantenimiento por empresas - Grafico 01
    $.ajax({
        type: 'POST',
