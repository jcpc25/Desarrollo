...
function actualizarFormatoHoja()
{
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: { idFormato:formato,idHoja:1, nombre: nombre, area: area, resolucion: resolucion, horizonte: horizonte, periodo: periodo, lectura: lectura, tituloHoja: tituloHoja },
        cache: false,
...
function modificarPto(formato, hoja, punto, pos, limsup, activo) {

    $.ajax({
        type: 'POST',
        url: controlador + "formatomedicion/MostrarEditarPto",
        data: {
            formato: formato,
            hoja: hoja, punto: punto, pos: pos, limsup: limsup, activo: activo
        },
        success: function (evt) {
            $('#modificarPto').html(evt);
            setTimeout(function () {
                $('#popupmpto').bPopup({
                    easing: 'easeOutBack',
                    speed: 450,
