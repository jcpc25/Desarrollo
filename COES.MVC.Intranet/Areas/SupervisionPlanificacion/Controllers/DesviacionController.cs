    ...
    public class DesviacionController : Controller
    {
        ...
        public ActionResult Grilla(String fecha)
        {
            ...
            try
            {
                ...
                if (list == null)
                {
                    model.ListaDesviaciones = new List<DesviacionDTO>();
                }
                else
                {
                    model.ListaDesviaciones = list;
                }
                model.fecha = DateTime.ParseExact(fecha, Constantes.FormatoFecha, CultureInfo.InvariantCulture);
        ...
        public JsonResult ProcesarArchivo(String fecha)
        {
            ...
            try
            {
                ...
                string path = AppDomain.CurrentDomain.BaseDirectory + Constantes.RutaCarga + Constantes.ArchivoDesviacion;
                list = (new RpdHelper()).LeerDesdeFormato(path);
                ...
                if (model.ListaDesviaciones != null)
                {
                    ...
                    entity.Desvfecha = DateTime.ParseExact(fecha.Substring(0, 10), Constantes.FormatoFecha, CultureInfo.InvariantCulture);
                    (new DesviacionAppServicio()).DeleteDesviacion((DesviacionDTO)entity);

                    for (int j = 0; j < (model.ListaDesviaciones).Count; j++)
                    {
                        fechaExcel = DateTime.ParseExact(fecha.Substring(0, 10), Constantes.FormatoFecha, CultureInfo.InvariantCulture);
