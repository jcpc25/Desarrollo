...
namespace COES.Infraestructura.Datos.Respositorio.Sic
{
    ...
    public class ExtLogenvioRepository : RepositoryBase, IExtLogenvioRepository
    {
        ...
        public List<ExtLogenvioDTO> GetByCriteria(int lectCodi, DateTime fecha, int? nroSemana)
        {
            List<ExtLogenvioDTO> entitys = new List<ExtLogenvioDTO>();
            if (nroSemana == null) nroSemana = 0;

            String query = String.Format(helper.SqlGetByCriteria, lectCodi, fecha.ToString(ConstantesBase.FormatoFecha), nroSemana);
