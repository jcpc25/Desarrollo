...
namespace WSIC2010.Util
{
    ...
    public static class Valida
    {
        ...
        public static bool NombreArchivo(string ps_codigoEmpr, string ps_nombreProg, string ps_nombreArch, string ps_extension, out string ps_formatoNombre, string ps_cadenaFecha)
        {
            DateTime ldt_fecha = new DateTime(2013, 1, 1);
            ...
            if (dt_empresas != null && dt_empresas.Rows.Count > 0)
            {
                empresa = new Empresa()
                {
                    ...
                    TipoEmpresaAbrev = (dt_empresas.Rows[0]["TIPOEMPRABREV"].ToString().Trim().Equals("ULIBRE")) ? "UL" : ((dt_empresas.Rows[0]["TIPOEMPRABREV"].ToString().Trim().Equals("DISTR")) ? "ED" : dt_empresas.Rows[0]["TIPOEMPRABREV"].ToString().Trim())
                };
            }
            else
            {
            ...
            if (ps_nombreProg.Equals("1"))
                ...
                ls_fecha = "SEM" + CargaDDLSemanas.LlenaSemanas(DateTime.Now.Date, out ldt_fecha).First().Key + ldt_fecha.ToString("yy");
