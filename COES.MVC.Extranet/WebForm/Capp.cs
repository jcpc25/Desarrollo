...
namespace WSIC2010
{
    public class n_app:fwapp.n_fw_app
    {
        public n_app()
        {
            iL_data[0] = new fwapp.n_fw_data(fwapp.DBType.OleDb, ConfigurationManager.ConnectionStrings["SICOES_OLEDB"].ToString());
            iL_data[1] = new fwapp.n_fw_data(fwapp.DBType.OleDb, ConfigurationManager.ConnectionStrings["SCADA_OLEDB"].ToString());
        }
